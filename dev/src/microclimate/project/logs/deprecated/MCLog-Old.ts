 ...
 */
export class MCLogOld implements vscode.QuickPickItem {
    ...
    protected constructor(
        ...
        public readonly logType: MCLogOld.LogTypes,
        private readonly managerMap: Map<string, MCLogOld>,
        initialMsg: string
    ) {
        const outputChannelName = Translator.t(MCLogOld.STRING_NS, "logName", { projectName, logType: logType.toString() });
    ...
    public stopUpdating(connectionLost: boolean = true): void {
        ...
        const msg = Translator.t(MCLogOld.STRING_NS, msgKey, { logType: this.logType.toString().toLowerCase() });
