 ...
 */
export default class MCLogManagerOld {
    ...
    public static doesSupportNewLogs(connection: Connection): boolean {
        return connection.version >= 1905;
    }

    /**
     * Returns an array of all open logs for this Connection (aka this logManager)
     */
    public getAllOpenLogs(): MCLogOld[] {
        let openLogs: MCLogOld[] = [];
    ...
    public getBuildLog(projectID: string): BuildLog | undefined {
        return this.buildLogMap.get(projectID);
    }

    public destroyAllLogs(projectID: string): void {
